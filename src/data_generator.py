...
class DataGeneratorFactory(object):
    ...
    def target2bboxes(self, y_true):
        """Convert target back to list of bounding boxes

        This is the inverse of the bboxes2target() method. The four-dimensional
        array y_true = y is assumed to contain the following information:
        ...
        Returns a list of bounding boxes, where each bounding box is represented by a
        dictionary encoding centre position (xc, yc), width and height in absolute coordinates.

        :arg y_true: four-dimensional array of shape (n_tiles,n_tiles,n_anchor,5+n_classes)
        """
        # Make sure that we no longer include the batch dimension
        assert len(np.shape(y_true)) == 4
        # Find all boxes actually associcated with an object
        tile_size = self.image_size // self.n_tiles
        confidence = y_true[..., 4]
        true_bbox_indices = np.transpose(np.where(confidence > 0))
        bboxes = []
        for true_bbox_index in true_bbox_indices:
            i_x, i_y, j = true_bbox_index
            bboxes.append(
                {
                    "xc": (float(y_true[i_x, i_y, j, 0]) + i_x) * tile_size,
                    "yc": (float(y_true[i_x, i_y, j, 1]) + i_y) * tile_size,
                    "width": float(y_true[i_x, i_y, j, 2])
                    / self.n_tiles
                    * self.image_size,
                    "height": float(y_true[i_x, i_y, j, 3])
                    / self.n_tiles
                    * self.image_size,
                    "class": np.argmax(y_true[i_x, i_y, j, 5:]),
                }
            )
        return bboxes
    ...
    def prediction2bboxes(self, y_pred, threshold=0.5):
        ...
        In addition to the bounding box coordinates, the dictionary encoding each
        bounding box also stored the confidence of the prediction.
        ...
        bboxes = [
            {
                ...
                "confidence": float(confidence_pred[i, j, k]),
            }
